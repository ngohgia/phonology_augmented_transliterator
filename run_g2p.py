...
def run_experiment(order, cwd):
    ...
    for M in xrange(1, order + 1):
        ...
        subprocess.Popen(['g2p.py'] + init + params , stdout=FNULL, stderr=FNULL).communicate()

        with open(hyp_file, 'w') as hyp_filehandler, open(output_file, 'w') as output_filehandler:
            out, err = subprocess.Popen(['g2p.py', '-m', modelFile, '-a', test_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
