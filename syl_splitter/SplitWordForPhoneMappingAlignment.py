...
from syl_struct_generator.SylStructGenerator import export_syl_struct_of_targ_word
...
def get_most_prob_hyps(hyps_list, search_space):
  # create all search coordinate for each letter
  ...
  return highest_compound_score_hyps
...
#------------------------ COMPARE SYL STRUCT ----------------------------#
def compare_syl_struct(reconstructed_word, targ_phones):
  syl_struct_str = export_syl_struct_of_targ_word(targ_phones)
  targ_syl_struct = [part.strip() for part in syl_struct_str.split('.')]

  if len(targ_syl_struct) != len(reconstructed_word):
    return False

  # evaluate syl struct of reconstructed_word
  for i in range(len(reconstructed_word)):
    new_syl_len = len(reconstructed_word[i])
    targ_syl_struct_len = len(targ_syl_struct[i].split())

    if new_syl_len != targ_syl_struct_len:
      return False
  return True
